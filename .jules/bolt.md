...
## 2025-10-25 - Avoid Sequential Query Execution in Data Aggregation Endpoints
**Learning:** Sequential parent-child queries (`await getJob(...)` followed by `await getPlan(...)` inside mapping functions or endpoints) create unnecessary latency. For endpoints aggregating entities, passing un-awaited promises to mapping/formatting functions (`toUiJob`) allows independent sub-queries (e.g. `toUiToolReceipts` and `getPlan`) to execute concurrently using `Promise.all`, significantly reducing the time it takes to build a large UI response payload.
**Action:** When optimizing API endpoints that aggregate entities, utilize `Promise.all` to fetch independent database records concurrently. Pass dependent queries as un-awaited promises to mapping/formatting functions to avoid parallelizing initial parent lookups with child queries when doing so would break 404 path handling.
