...
## 2024-07-28 - Add database index on frequently queried foreign key fields
**Learning:** SQLite does not automatically index foreign key columns. Leaving `job_id` and `receipt_id` unindexed in heavily queried tables (`plans`, `receipts`, `results`, `approvals`) causes expensive O(N) full table scans, particularly during concurrent `Promise.all` lookups.
**Action:** When creating or modifying SQLite schema definitions, always explicitly pair `FOREIGN KEY` definitions with corresponding `CREATE INDEX` statements to prevent full table scans and improve query latency.
