...
## 2024-08-02 - SQLite Missing Indexes on Foreign Keys in memory store
**Learning:** SQLite does not automatically create indexes on `FOREIGN KEY` columns. Without them, queries fetching related records (like `getPlan` by `job_id` or `getResultForReceipt` by `receipt_id`) in `Promise.all` loops perform O(N) full table scans, causing O(N^2) bottlenecks on endpoints like `/api/jobs`.
**Action:** Always pair `FOREIGN KEY` definitions with `CREATE INDEX` statements in SQLite schemas to ensure efficient lookups and prevent O(N) full table scans.
