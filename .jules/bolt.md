...
## 2026-07-30 - Missing SQLite Foreign Key Indexes
**Learning:** SQLite requires explicit indexes on foreign keys to prevent O(N) full table scans, especially when these tables are queried frequently in a Promise.all block.
**Action:** Always create explicitly paired CREATE INDEX statements alongside FOREIGN KEY definitions.
