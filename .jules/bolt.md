...
## 2025-06-03 - API route 500 error from over-parallelization
**Learning:** Parallelizing the initial parent lookup (e.g., `getJob`) with child queries using a raw route ID parameter causes 500 errors on 404 paths because the parent might not exist.
**Action:** Instead of parallelizing everything at the route level, verify the parent first or pass dependent queries as un-awaited promises to mapping/formatting functions (like `toUiJob`) to resolve them concurrently without risking 500 errors on missing parents.
