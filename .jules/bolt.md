...
## 2024-08-09 - Missing SQLite Indices for Foreign Keys
**Learning:** SQLite does not automatically create indices for FOREIGN KEY columns. Unindexed foreign keys can cause O(N) full table scans when querying child records by parent ID (e.g., getting plans, receipts, approvals by `job_id`), especially in `Promise.all` loops.
**Action:** Always explicitly pair `FOREIGN KEY` definitions with corresponding `CREATE INDEX` statements for tables frequently queried by parent ID.
