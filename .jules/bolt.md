...
## 2026-07-31 - SQLite Foreign Key Indexes
**Learning:** Missing foreign key indexes in SQLite schemas leads to O(N) full table scans during concurrent Promise.all loops (like querying results for receipts).
**Action:** Always explicitly pair FOREIGN KEY definitions with corresponding CREATE INDEX statements to prevent performance bottlenecks.
