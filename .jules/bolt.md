...
## 2025-07-20 - Missing explicit index on foreign keys queried in Promise.all
**Learning:** In SQLite schemas, explicit `CREATE INDEX` statements must be paired with `FOREIGN KEY` definitions, especially for tables heavily queried in `Promise.all` aggregation loops (e.g., retrieving related plans, receipts, and results concurrently for multiple jobs). Without these indexes, each concurrent sub-query triggers an O(N) full table scan, severely bottlenecking thread performance.
**Action:** Always explicitly define `CREATE INDEX` alongside `FOREIGN KEY` declarations in SQLite schemas to prevent performance degradation under concurrent read loads.
