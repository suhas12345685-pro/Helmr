...
## 2025-06-03 - Optimize sequential database reads in API endpoints
**Learning:** In endpoints that return aggregated data (like `/api/jobs/:id` or `/api/approvals`), fetching related entities sequentially (e.g., `await store.getJob()`, then `await store.getPlan()`) creates a "waterfall" effect, doubling the latency.
**Action:** Use `Promise.all` to fetch independent database records (like job and plan) concurrently in API endpoints and formatting functions to reduce total response latency.
