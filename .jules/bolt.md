...
## 2025-06-03 - SQLite Foreign Key Indexes & API Aggregation Bottlenecks
**Learning:** In SQLite, explicitly defining a `FOREIGN KEY` does not automatically create an index on that column. Queries filtering by these foreign keys (or heavily queried non-foreign-key columns) result in O(N) full table scans. Furthermore, fetching dependent but independent relations sequentially in API endpoints compounds latency.
**Action:** Always pair `FOREIGN KEY` definitions with `CREATE INDEX` statements in SQLite. Utilize `Promise.all` to fetch independent database records (e.g., job, plan, receipts) concurrently rather than sequentially to reduce response latency.
