...
## 2025-08-27 - Optimize API endpoints by parallelizing DB queries and adding indexes
**Learning:** When aggregating entities in API endpoints (like `toUiJob` and `/api/approvals`), fetching independent database records sequentially creates unnecessary latency. In addition, missing indexes on foreign key columns (like `job_id` in `plans`, `receipts`, `results`, `approvals`) can lead to O(N) full table scans when these tables grow. However, avoid parallelizing the initial parent lookup with child queries using a raw route ID parameter to prevent 500 errors on 404 paths.
**Action:** Use `Promise.all` to fetch independent data concurrently (e.g., passing un-awaited promises to mapping/formatting functions), and explicitly pair `FOREIGN KEY` definitions with `CREATE INDEX` statements in SQLite schemas.
