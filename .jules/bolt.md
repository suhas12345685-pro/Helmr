...
## 2026-08-15 - Optimize Hatchery API entity aggregation
**Learning:** Endpoints like GET /api/jobs and GET /api/approvals aggregate independent data models (jobs, plans, tool receipts). By passing dependent queries as un-awaited promises to mapping/formatting functions (`toUiJob`), we can utilize `Promise.all` internally to fetch records concurrently, avoiding sequential fetching bottlenecks. However, when querying with user-provided IDs (e.g. GET /api/jobs/:id), the initial parent lookup (`getJob`) must be verified first before fetching child records (like plans) to avoid unnecessary queries and potential 500 errors on 404 paths.
**Action:** Use `Promise.all` and un-awaited promises to fetch independent database records concurrently in mapping functions to reduce latency, while ensuring initial parent lookup is not parallelized for specific single-entity routes to handle 404 paths correctly.
