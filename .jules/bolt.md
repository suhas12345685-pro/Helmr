...
## 2025-06-03 - Optimize approval list fetching
**Learning:** Sequential `await` calls for independent database queries (like `getJob` and `getPlan`) inside map loops can double query latency for endpoint responses like `/api/approvals`.
**Action:** Replace sequential queries with `Promise.all` to fetch related records concurrently when constructing list responses.
