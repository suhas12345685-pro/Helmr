...
## 2024-05-24 - SQLite Promise.all N+1 Optimization
**Learning:** Querying SQLite tables using `Promise.all` without indexes on foreign keys causes O(N) full table scans, destroying performance in aggregator endpoints (like `/api/jobs`).
**Action:** Always explicitly pair `FOREIGN KEY` definitions with `CREATE INDEX` statements, especially for dependent entities fetched concurrently.
