...
## 2026-06-03 - Avoid sequential N+1 database waterfalls in API aggregators
**Learning:** When building or optimizing API endpoints that aggregate multiple entities (like `toUiJob` fetching plan and tool receipts sequentially for each job), using sequential `await` calls creates massive N+1 latency.
**Action:** Utilize `Promise.all` to fetch independent database records (e.g., job, plan, receipts) concurrently rather than sequentially to significantly reduce response latency.
