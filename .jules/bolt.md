...
## 2024-08-20 - Missing SQLite Foreign Key Indexes
**Learning:** SQLite does not automatically index foreign keys, leading to O(N) full table scans when queried by parent IDs, especially in concurrent Promise.all loops mapping over relations.
**Action:** Always explicitly pair FOREIGN KEY definitions with CREATE INDEX statements to guarantee indexed lookups for relation resolution.
