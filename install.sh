...
# Helmr Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://helmr.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Helmr."
...
resolve_helmr_effective_home() {
    local helmr_home="${HELMR_HOME:-}"
    if [[ -z "$helmr_home" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$helmr_home" == "~" ]]; then
...
GUM_VERSION="${HELMR_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Helmr Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://helmr.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$HELMR_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.helmr.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_helmr_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.helmr-* "$npm_root"/helmr 2>/dev/null || true
}

extract_helmr_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_helmr_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/helmr" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/helmr"|"/usr/local/bin/helmr")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/helmr/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale helmr symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing helmr binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Helmr package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi

    ui_info "Installing Helmr package"
    "${cmd[@]}" >"$log" 2>&1
}
...
install_helmr_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Helmr npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*helmr" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_helmr_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_helmr_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Helmr npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an helmr binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${HELMR_TAGLINE_INDEX:-}" ]]; then
        if [[ "${HELMR_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((HELMR_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${HELMR_NO_ONBOARD:-0}
NO_PROMPT=${HELMR_NO_PROMPT:-0}
DRY_RUN=${HELMR_DRY_RUN:-0}
INSTALL_METHOD=${HELMR_INSTALL_METHOD:-}
HELMR_VERSION=${HELMR_VERSION:-latest}
USE_BETA=${HELMR_BETA:-0}
GIT_DIR_DEFAULT="$(resolve_helmr_effective_home)/helmr"
GIT_DIR=${HELMR_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${HELMR_GIT_UPDATE:-1}
NPM_LOGLEVEL="${HELMR_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${HELMR_VERBOSE:-0}"
VERIFY_INSTALL="${HELMR_VERIFY_INSTALL:-0}"
HELMR_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Helmr installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://helmr.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/helmr)
...
Environment variables:
  HELMR_INSTALL_METHOD=git|npm
  HELMR_VERSION=latest|next|<semver>|<spec>
  HELMR_BETA=0|1
  HELMR_GIT_DIR=...
  HELMR_GIT_UPDATE=0|1
  HELMR_NO_PROMPT=1
  HELMR_VERIFY_INSTALL=1
  HELMR_DRY_RUN=1
  HELMR_NO_ONBOARD=1
  HELMR_VERBOSE=1
  HELMR_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://helmr.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://helmr.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://helmr.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://helmr.ai/install.sh | bash -s -- --install-method git --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://helmr.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Helmr checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Helmr source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_helmr_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"helmr"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://helmr.ai/install.sh | bash"
}
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future Helmr updates; use npm i -g helmr@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_helmr_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/helmr" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/helmr" ]]; then
        ln -sf "$npm_root/helmr/dist/entry.js" "${npm_bin}/helmr"
        ui_info "Created helmr bin link at ${npm_bin}/helmr"
    fi
    return 0
}

# Check for existing Helmr installation
check_existing_helmr() {
    if [[ -n "$(type -P helmr 2>/dev/null || true)" ]]; then
        ui_info "Existing Helmr installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_helmr_ref() {
    local requested="${HELMR_VERSION:-latest}"
    local resolved_version=""

    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "helmr" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_helmr_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${HELMR_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_helmr_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/helmr"
        ...
        local version=""
        version="$(helmr_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_helmr_npm_root_candidates)
}

warn_duplicate_helmr_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_helmr_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple Helmr global installs detected"
    echo "  Different Node/npm environments can run different Helmr versions."

    local active_node active_npm active_helmr
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_helmr="${HELMR_BIN:-}"
    if [[ -z "$active_helmr" ]]; then
        active_helmr="$(type -P helmr 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active helmr: ${INFO}${active_helmr:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make helmr show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

helmr_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "helmr"
        return 0
...
bounded_probe_output() {
    local label="$1"
    shift
    local timeout_seconds="${HELMR_INSTALL_PROBE_TIMEOUT_SECONDS:-5}"
...
warn_helmr_not_found() {
    ui_warn "Installed, but helmr is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t helmr 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named helmr; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_helmr_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P helmr 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/helmr" ]]; then
        echo "${npm_bin}/helmr"
        return 0
    fi
...
install_helmr_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/helmr/helmr.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Helmr from git checkout: ${repo_dir}"
    else
        ui_info "Installing Helmr from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        ...
        run_quiet_step "Cloning Helmr" git clone "$repo_url" "$repo_dir"
    fi

    local git_ref
    git_ref="$(resolve_git_helmr_ref)"
    if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
        ui_info "Using git ref: ${git_ref}"
        checkout_git_helmr_ref "$repo_dir" "$git_ref"
    else
    ...
    run_quiet_step "Building Helmr" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/helmr" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/helmr"
    ui_success "Helmr wrapper installed to \$HOME/.local/bin/helmr"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view helmr dist-tags.beta 2>/dev/null || true)"
...
is_helmr_source_package_install_spec() {
    local value="${1:-}"
    local normalized_value=""
    normalized_value="$(to_lowercase_ascii "$value")"
    normalized_value="${normalized_value#helmr@}"

    [[ "$normalized_value" == "main" ]] && return 0
    [[ "$normalized_value" =~ ^github:helmr/helmr($|[#/]) ]] && return 0

    normalized_value="${normalized_value#git+}"
    [[ "$normalized_value" =~ ^https?://github\.com/helmr/helmr(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^ssh://git@github\.com[:/]helmr/helmr(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git://github\.com/helmr/helmr(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git@github\.com:helmr/helmr(\.git)?($|[?#]) ]] && return 0
    return 1
}
...
install_helmr() {
    local package_name="helmr"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            HELMR_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="helmr"
        else
            HELMR_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${HELMR_VERSION}" ]]; then
        HELMR_VERSION="latest"
    fi

    if is_helmr_source_package_install_spec "${HELMR_VERSION}"; then
        ui_error "npm installs do not support Helmr GitHub source targets like '${HELMR_VERSION}'."
    ...
    local resolved_version=""
    if can_resolve_registry_package_version "${HELMR_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${HELMR_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Helmr v${resolved_version}"
    else
        ui_info "Installing Helmr (${HELMR_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${HELMR_VERSION}")"

    if ! install_helmr_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_helmr_paths
        install_helmr_npm "${install_spec}"
    fi

    if [[ "${HELMR_VERSION}" == "latest" && "${package_name}" == "helmr" ]]; then
        if ! resolve_helmr_bin &> /dev/null; then
            ui_warn "npm install helmr@latest failed; retrying helmr@next"
            cleanup_npm_helmr_paths
            install_helmr_npm "helmr@next"
        fi
    fi

    ensure_helmr_bin_link || true

    ui_success "Helmr installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${HELMR_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_helmr_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (helmr not on PATH yet)"
        warn_helmr_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${HELMR_PROFILE:-default}"
    local effective_home
    effective_home="$(resolve_helmr_effective_home)"
    if [[ "${profile}" != "default" ]]; then
        echo "${effective_home}/.helmr/workspace-${profile}"
    else
        echo "${effective_home}/.helmr/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local effective_home
    effective_home="$(resolve_helmr_effective_home)"
    local config_path="${HELMR_CONFIG_PATH:-$effective_home/.helmr/helmr.json}"
    local legacy_config_path="${HOME}/.helmr/helmr.json"
    local legacy_clawdbot_path="${HOME}/.clawdbot/clawdbot.json"
    if [[ -f "${config_path}" || -f "$effective_home/.clawdbot/clawdbot.json" ]]; then
        return
    fi
    if [[ -z "${HELMR_CONFIG_PATH:-}" && "${effective_home}" != "${HOME}" ]]; then
    ...
    if ! is_promptable; then
        local user_claw
        user_claw="$(helmr_command_for_user "${HELMR_BIN:-}")"
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but helmr not on PATH; skipping onboarding"
        warn_helmr_not_found
        return
    fi
    ...
    "$claw" onboard || {
        local user_claw
        user_claw="$(helmr_command_for_user "$claw")"
...
resolve_helmr_version() {
    local version=""
    local raw_version_output=""
    local claw="${HELMR_BIN:-}"
    if [[ -z "$claw" ]] && command -v helmr &> /dev/null; then
        claw="$(command -v helmr)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_helmr_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/helmr/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/helmr/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$(bounded_probe_output "helmr daemon status --json" "$claw" daemon status --json || true)"
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: helmr not on PATH yet"
        warn_helmr_not_found
        return 1
    fi

    run_quiet_step "Checking Helmr version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: helmr gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_helmr_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Helmr checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set HELMR_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_helmr_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/helmr" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/helmr"
    ...
    HELMR_BIN="$(resolve_helmr_bin || true)"
    warn_duplicate_helmr_global_installs || true
    ...
    installed_version=$(resolve_helmr_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Helmr installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Helmr installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/helmr"
        ui_kv "Update command" "helmr update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://helmr.ai/install.sh | bash -s -- --install-method npm"
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            ui_info "Running helmr doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                HELMR_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                HELMR_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                HELMR_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
        ...
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if is_promptable; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (helmr not on PATH yet)"
                    warn_helmr_not_found
                    return 0
                fi
                exec </dev/tty
    ...
    if command -v helmr &> /dev/null; then
        local claw="${HELMR_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_helmr_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (helmr daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if HELMR_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: helmr daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${HELMR_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_install_stage_total
    configure_verbose
    main
