...
if (!String(process.env.HELMR_AUDIT_DIR ?? '').trim() && !String(process.env.HELMR_AUDIT_LOG ?? '').trim()) {
...
const deployOrigins = String(process.env.HELMR_ALLOWED_ORIGINS ?? '').split(',').map((part) => part.trim()).filter(Boolean);
if (deployOrigins.some((origin) => origin.includes('*'))) failures.push('HELMR_ALLOWED_ORIGINS cannot contain wildcard origins for deployment verification.');
