# Helmr Installer for Windows
# Usage: powershell -c "irm https://helmr.ai/install.ps1 | iex"
#        powershell -c "& ([scriptblock]::Create((irm https://helmr.ai/install.ps1))) -Tag beta -NoOnboard -DryRun"
...
function Complete-Install {
    ...
    throw "Helmr installation failed with exit code $($script:InstallExitCode)."
}

Write-Host ""
Write-Host "  Helmr Installer" -ForegroundColor Cyan
Write-Host ""
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:HELMR_INSTALL_METHOD)) {
        $InstallMethod = $env:HELMR_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:HELMR_GIT_DIR)) {
        $GitDir = $env:HELMR_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:HELMR_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:HELMR_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:HELMR_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    ...
    $GitDir = (Join-Path $userHome "helmr")
}
...
function Get-HelmrDepsRoot {
    ...
    return (Join-Path $localAppData "Helmr\deps")
}

function Get-PortableNodeRoot {
    return (Join-Path (Get-HelmrDepsRoot) "portable-node")
}
...
function Ensure-PortableNodeOnUserPath {
    ...
    if (Add-ToUserPath $nodeDir) {
        Write-Host "[!] Added $nodeDir to user PATH (restart terminal if node or helmr is not found)" -ForegroundColor Yellow
    }
}
...
function Check-ExistingHelmr {
    if (Get-HelmrCommandPath) {
        Write-Host "[*] Existing Helmr installation detected" -ForegroundColor Yellow
        return $true
    }
    return $false
}
...
function Get-PortableGitRoot {
    return (Join-Path (Get-HelmrDepsRoot) "portable-git")
}
...
function Install-PortableGit {
    ...
    $portableParent = Split-Path -Parent $portableRoot
    $tmpZip = Join-Path $env:TEMP $download.Name
    $tmpExtract = Join-Path $env:TEMP ("helmr-portable-git-" + [guid]::NewGuid().ToString("N"))
...
function Ensure-Git {
    ...
    Write-Host ""
    Write-Host "Error: Git is required to install Helmr." -ForegroundColor Red
...
function Get-HelmrCommandPath {
    $helmrCmd = Get-Command helmr.cmd -ErrorAction SilentlyContinue
    if ($helmrCmd -and $helmrCmd.Source) {
        return $helmrCmd.Source
    }

    $helmr = Get-Command helmr -ErrorAction SilentlyContinue
    if ($helmr -and $helmr.Source) {
        return $helmr.Source
    }
...
function Invoke-HelmrCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )

    $commandPath = Get-HelmrCommandPath
    if (-not $commandPath) {
        throw "helmr command not found on PATH."
    }

    & $commandPath @Arguments
}

function Invoke-InteractiveHelmrCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
    ...
    if ($process.ExitCode -ne 0) {
        throw "helmr $($Arguments -join ' ') failed with exit code $($process.ExitCode)."
    }
}
...
function Ensure-HelmrOnPath {
    ...
    foreach ($npmBin in $npmBins) {
        if (-not (Test-Path (Join-Path $npmBin "helmr.cmd"))) {
            continue
        }
    ...
    Write-Host "[!] helmr is not on PATH yet." -ForegroundColor Yellow
...
function Resolve-NpmHelmrInstallSpec {
    param(
        [string]$PackageName,
        [string]$RequestedTag
    )
...
function Test-HelmrSourcePackageInstallSpec {
    ...
    if ($normalizedTag.StartsWith("helmr@")) {
        $normalizedTag = $normalizedTag.Substring("helmr@".Length)
    }

    if ($normalizedTag -eq "main") {
        return $true
    }
    if ($normalizedTag -match '^github:helmr/helmr($|[#/])') {
        return $true
    }
    ...
    return (
        $normalizedTag -match '^https?://github\.com/helmr/helmr(\.git)?($|[?#])' -or
        $normalizedTag -match '^ssh://git@github\.com[:/]helmr/helmr(\.git)?($|[?#])' -or
        $normalizedTag -match '^git://github\.com/helmr/helmr(\.git)?($|[?#])' -or
        $normalizedTag -match '^git@github\.com:helmr/helmr(\.git)?($|[?#])'
    )
}
...
function Install-Helmr {
    if ([string]::IsNullOrWhiteSpace($Tag)) {
        $Tag = "latest"
    }
    if (Test-HelmrSourcePackageInstallSpec -RequestedTag $Tag) {
        Write-Host "Error: npm installs do not support Helmr GitHub source targets like '$Tag'." -ForegroundColor Red
    ...
    # Use helmr package for beta, helmr for stable
    $packageName = "helmr"
    if ($Tag -eq "beta" -or $Tag -match "^beta\.") {
        $packageName = "helmr"
    }
    $installSpec = Resolve-NpmHelmrInstallSpec -PackageName $packageName -RequestedTag $Tag
    Write-Host "[*] Installing Helmr ($installSpec)..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host '  powershell -c "irm https://helmr.ai/install.ps1 | iex"' -ForegroundColor Cyan
            }
    ...
    Write-Host "[OK] Helmr installed" -ForegroundColor Green
    return $true
}

# Install Helmr from GitHub
function Install-HelmrFromGit {
    param(
        [string]$RepoDir,
        [switch]$SkipUpdate
    )
    ...
    $repoUrl = "https://github.com/helmr/helmr.git"
    Write-Host "[*] Installing Helmr from GitHub ($repoUrl)..." -ForegroundColor Yellow
    ...
    if (-not (Test-Path $entryPath)) {
        Write-Host "[!] Helmr build did not produce $entryPath" -ForegroundColor Red
        return $false
    }
    ...
    $cmdPath = Join-Path $binDir "helmr.cmd"
    ...
    Write-Host "[OK] Helmr wrapper installed to $cmdPath" -ForegroundColor Green
...
function Run-Doctor {
    ...
    try {
        Invoke-HelmrCommand doctor --non-interactive
    } catch {
...
function Test-GatewayServiceLoaded {
    try {
        $statusJson = (Invoke-HelmrCommand daemon status --json 2>$null)
...
function Refresh-GatewayServiceIfLoaded {
    ...
    try {
        Invoke-HelmrCommand gateway install --force | Out-Null
    } catch {
    ...
    try {
        Invoke-HelmrCommand gateway restart | Out-Null
        Invoke-HelmrCommand gateway status --json | Out-Null
...
function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:HELMR_GIT_DIR)) {
        return $env:HELMR_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "helmr")
}
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        try {
            ...
            if ($npmCommand) {
                Invoke-NpmCommand -Arguments @("uninstall", "-g", "helmr") 2>$null | Out-Null
                Write-Host "[OK] Removed npm global install if present" -ForegroundColor Green
            }
        } catch { }
        $finalGitDir = $GitDir
        $gitInstallResults = @(Install-HelmrFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate)
        if (-not (Test-BooleanSuccessResult -Results $gitInstallResults)) {
            return (Fail-Install)
        }
    } else {
        $gitWrapper = Join-Path (Join-Path $env:USERPROFILE ".local\\bin") "helmr.cmd"
        ...
        $npmInstallResults = @(Install-Helmr)
        if (-not (Test-BooleanSuccessResult -Results $npmInstallResults)) {
            return (Fail-Install)
        }
    }

    if (-not (Ensure-HelmrOnPath)) {
        Write-Host "Install completed, but Helmr is not on PATH yet." -ForegroundColor Yellow
        Write-Host "Open a new terminal, then run: helmr doctor" -ForegroundColor Cyan
        return
    }
    ...
    try {
        $installedVersion = (Invoke-HelmrCommand --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.helmr -and $npmList.dependencies.helmr.version) {
                $installedVersion = $npmList.dependencies.helmr.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    Write-Host ""
    if ($installedVersion) {
        Write-Host "Helmr installed successfully ($installedVersion)!" -ForegroundColor Green
    } else {
        Write-Host "Helmr installed successfully!" -ForegroundColor Green
    }
    Write-Host ""
    if ($isUpgrade) {
        $updateMessages = @(
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\helmr.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "helmr doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "helmr onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
            Write-Host "Starting setup..." -ForegroundColor Cyan
            Write-Host ""
            Invoke-InteractiveHelmrCommand onboard
        }
    }
