...
async function toUiJob(store: HelmrSQLiteStore, job: HelmrStoreJob, planPromise: Promise<HelmrPlan | undefined> | HelmrPlan | null | undefined) {
...
export function createHatcheryApp(
  ...
  app.get('/api/jobs/:id', async (c) => {
    ...
    return c.json({ job: await toUiJob(store, job, plan), plan: plan ?? null });
  });
