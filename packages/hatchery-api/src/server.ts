...
async function toUiJob(store: HelmrSQLiteStore, job: HelmrStoreJob, plan?: HelmrPlan | null, toolReceipts?: any[]) {
  return {
    id: job.id,
    ...
    toolReceipts: toolReceipts ?? await toUiToolReceipts(store, job.id),
  };
}
...
export function createHatcheryApp(
  ...
  app.get('/api/jobs', async (c) => {
    ...
    const withPlans = await Promise.all(
      jobs.map(async (job) => {
        ...
        return toUiJob(store, job, plan ?? null, toolReceipts);
      })
    );
  ...
  app.get('/api/jobs/:id', async (c) => {
    ...
    return c.json({ job: await toUiJob(store, job, plan ?? null, toolReceipts), plan: plan ?? null });
  });
