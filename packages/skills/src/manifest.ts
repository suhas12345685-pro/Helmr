...
export const SkillManifestSchema = z.object({
  ...
  healthCheck: z.object({ command: z.string().optional(), url: z.string().url().optional(), timeoutMs: z.number().int().positive().default(5000) }).default({}),
  runtimeConstraints: z.object({ network: z.enum(['none', 'approval', 'allowed']).default('approval'), maxRuntimeMs: z.number().int().positive().optional(), requiresWorkspace: z.boolean().default(false) }).default({}),
