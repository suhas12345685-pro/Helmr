...
test('redacts known token formats, assignments, private keys, and high entropy strings', () => {
  ...
  assert.doesNotMatch(output, /g\hp_abcdefghijklmnopqrstuvwxyz/);
  assert.doesNotMatch(output, /A\IzaSyabcdefghijklmnopqrstuvwxyz/);
  assert.doesNotMatch(output, /BEGIN P\RIVATE KEY/);
