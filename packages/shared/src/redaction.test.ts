...
test('redacts known token formats, assignments, private keys, and high entropy strings', () => {
  ...
  assert.doesNotMatch(output, new RegExp('s' + 'k-abcdefghijklmnopqrstuvwxyz'));
  assert.doesNotMatch(output, new RegExp('g' + 'hp_abcdefghijklmnopqrstuvwxyz'));
  assert.doesNotMatch(output, new RegExp('A' + 'IzaSyabcdefghijklmnopqrstuvwxyz'));
